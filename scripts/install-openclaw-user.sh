...
if [ "$USER" != "openclaw" ]; then
    ...
    read_input "是否仍要继续? (y/N): " continue_anyway "N"
    if [[ ! "$continue_anyway" =~ ^[Yy]$ ]]; then
        log_info "请切换到 openclaw 用户后重新运行: su - openclaw"
        exit 0
    fi
fi

log_info "=========================================="
log_info " OpenClaw-Termux 安装脚本 - 应用安装阶段"
log_info "=========================================="
echo ""
log_ok "当前用户: $USER"
log_ok "家目录: $HOME"
...
log_info "2/3 安装 OpenClaw-CN-Termux..."
