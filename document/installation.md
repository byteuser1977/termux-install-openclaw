...
本文档介绍在 Termux 中通过 **proot-distro 运行 Ubuntu 系统**来安�?OpenClaw-CN。这是推荐的安装方式，可获得完整�?Linux 环境和更好的兼容性�?
---
...
OpenClaw是一个开源的 AI 智能助手系统，支持多渠道消息接入和灵活的技能扩展。在 Android 上，我们通过 Termux 应用配合 proot-distro 提供完整�?Linux 用户空间环境，实现与服务器部署相同的体验
### 预期时间
...
** F-Droid 下载最新版 Termux ** https://github.com/termux/termux-app/releases
...
> 💡 **提示**: 首次安装会下载约 200-300 MB �?rootfs 镜像。可以使用国内镜像加速：
> ```bash
...
OpenClaw 需要 Node.js 22 或更高版本。我们使�?nvm（Node Version Manager）安装，便于版本管理和更新
### 5.1 切换到 openclaw 用户
...
向导会引导你�?1. 选择语言（中�?English�?2. 配置网关端口（默�?18789�?3. 设置管理�?token
4. 选择启用的渠道插�?5. 配置 AI 模型（API Key、模型名称）
...
**关键字段说明**�?- `agent.apiKey`: AI 模型�?API Key（必填）
- `agent.model`: 使用的模型名�?- `gateway.port`: 网关监听端口（默�?18789�?- `gateway.token`: 访问网关的令牌（用于客户端连接）
- `gateway.bind`: 绑定地址，`lan` 表示监听所有网络接�?
### 7.3 启用人脸识别或渠道插�?
如需使用特定渠道（如飞书、Telegram），�?`plugins` 部分配置�?
```json
{
  "plugins": {
    "feishu": {
...
# 测试网关健康检查端�?curl http://localhost:18789/health
...
### 问题 1: 命令 `openclaw-termux` 找不�?
**原因**: npm 全局 bin 目录不在 PATH 中�?
**解决**:
```bash
# 查看 npm 全局安装位置
npm config get prefix
# 通常�?/home/openclaw/.npm-global �?/usr/local

# 添加�?PATH（编�?~/.bashrc�?export PATH="$HOME/.npm-global/bin:$PATH"
...
**原因**:
- gateway.bind 设置�?"localhost"
- 防火墙限�?- Ubuntu 容器网络配置问题

**解决**:
1. 确保配置�?`"bind": "lan"` �?`"bind": "0.0.0.0"`
2. 检�?Ubuntu 容器 IP 并确保可访问�?   ```bash
   hostname -I
   ```
3. 如有防火墙，开放端口：
   ```bash
...
**症状**: 日志中显�?`Cannot find module '@xxx/plugin'`
...
Termux �?Ubuntu 容器内存不足时，Node.js 可能�?OOM kill�?
**解决**:
1. 关闭不必要的应用
2. 增加 swap 空间（在 Ubuntu 中）�?   ```bash
...
```bash
# 打包配置和数�?tar -czf ~/openclaw-backup-$(date +%Y%m%d).tar.gz ~/.openclaw/
...
- **Termux 开发�?* - 强大�?Android 终端环境
- **proot-distro** - �?Android 上运�?Linux 发行�?
---

**遇到问题�?* 查看 [troubleshooting.md](./troubleshooting.md) 或提�?[GitHub Issue](https://github.com/jiulingyun/openclaw-cn/issues)�?
...
` 
�ýű����Զ���� Termux �������ú� Ubuntu ��װ��

### ʹ�� install-openclaw.sh

�� Ubuntu �����У�openclaw �û�����
`ash
./scripts/install-openclaw.sh
` 
�ýű����Զ���װ Node.js��pnpm �� OpenClaw-CN��
